...
class RebalanceScreen extends StatefulWidget {
  ...
  @override
  _RebalanceScreenState createState() => _RebalanceScreenState(); // Crée l'état associé
}
...
class _RebalanceScreenState extends State<RebalanceScreen> {
  ...
  void initState() {
    ...
    if (orderProvider.rebalanceRecommendations.isEmpty) {
       ...
       orderProvider.fetchRebalanceRecommendations(); // Appelle la méthode du Provider pour charger les données
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    final orderProvider = Provider.of<OrderProvider>(context); // Écoute le provider pour les changements

    return Padding( // Garde le padding comme conteneur racine
      padding: const EdgeInsets.all(16.0),
      child: Column( // Colonne principale
        crossAxisAlignment: CrossAxisAlignment.stretch, // Étire les enfants (texte, liste, bouton)
        children: [
          const Text(
          ...
          Expanded( // Permet à la liste de prendre l'espace restant
            child: orderProvider.isLoading && orderProvider.rebalanceRecommendations.isEmpty // Affiche spinner si chargement ET liste vide (pour ne pas cacher le cache)
                ? const Center(child: CircularProgressIndicator())
                : orderProvider.errorMessage != null && orderProvider.rebalanceRecommendations.isEmpty // Affiche erreur si erreur ET liste vide
                    ? Center(
                       child: Column( // Centre le contenu de l'erreur
                          mainAxisAlignment: MainAxisAlignment.center,
                          children: [
                             const Icon(Icons.error_outline, color: Colors.red, size: 50),
                             const SizedBox(height: 10),
                             Text(
                                orderProvider.errorMessage!, // Affiche le message d'erreur du provider
                                textAlign: TextAlign.center,
                                style: const TextStyle(color: Colors.red, fontSize: 16),
                             ),
                             const SizedBox(height: 20),
                             ElevatedButton(
                                onPressed: () => orderProvider.fetchRebalanceRecommendations(), // Bouton Retry
                                child: const Text("Réessayer"),
                             ),
                          ],
                       ),
                     )
                    : orderProvider.rebalanceRecommendations.isEmpty // Affiche message vide si liste vide (et pas en chargement ou erreur gérée)
                        ? const Center(child: Text("Aucune recommandation de rééquilibrage disponible pour l'instant."))
                        : ListView.builder( // Affiche la liste si des recommandations sont présentes
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return Card(
                                margin: const EdgeInsets.symmetric(vertical: 4.0), // Marge verticale entre les cartes
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        recommendation.assetName ?? 'N/A', // Utilise N/A si le nom est null
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          Text('Current: ${recommendation.currentPercentage ?? 'N/A'}%'), // Gère null
                                          Text('Target: ${recommendation.targetPercentage ?? 'N/A'}%'), // Gère null
                                        ],
                                      ),
                                      const SizedBox(height: 8),
                                      ...
                                      Text(
                                        ...
                                        style: TextStyle(
                                          color: recommendation.amountToAdjust != null
                                            ? (recommendation.amountToAdjust! > 0 ? Colors.green : Colors.red)
                                            : Colors.grey, // Couleur grise si la valeur est null
             ...
             onPressed: () async {
              ...
              // Ton code existant utilisant mounted et OrderProvider
               final orders = orderProvider.rebalanceRecommendations.map((rec) {
                 ...
                 return Order( // <-- Assure-toi que Order est importé/défini
                   id: '', // L'ID devrait probablement être généré ou non requis ici
                   assetId: rec.assetId ?? '', // Gère les valeurs nulles si elles peuvent l'être
                   assetName: rec.assetName ?? 'N/A',
                   amount: rec.amountToAdjust != null ? rec.amountToAdjust!.abs() : 0, // Gère null
                   price: 0, // Le prix au moment de l'ordre est important
                   date: DateTime.now(),
                   type: rec.amountToAdjust != null ? (rec.amountToAdjust! > 0 ? OrderType.buy : OrderType.sell) : OrderType.buy, // Gère null, type par défaut si null
                 );
               }).toList();
               ...
               if (success && mounted) { // <-- 'mounted' est maintenant valide !
                 ...
                 // orderProvider.fetchRebalanceRecommendations(); // Pour recharger après exécution
                  ...
                  // Gérer l'échec de l'exécution (afficher un message d'erreur par exemple)
                   ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Échec de l\'exécution du rééquilibrage.')),
                   );
               }
             },
