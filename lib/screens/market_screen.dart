...
class _MarketScreenState extends State<MarketScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Marché des Actifs')), // Titre de l barre d'applis
      body: RefreshIndicator( // Permet le "pull-to-refresh"
        onRefresh: () => provider.fetchAssets(), // Appelle la fonction de chargement lors du rafraîchissement
        child: _buildBody(provider), // Méthode pour construire le contenu dynamique
      ),
    );
  }
  ...
  Widget _buildBody(MarketDataProvider provider) {
             ...
             child: Column(
               ...
               children: [
                 ...
                 Text(
                  ...
                  ElevatedButton(
                     onPressed: () => provider.fetchAssets(), // Bouton pour réessayer l'appel API
     ...
     return ListView.builder(
       ...
       itemBuilder: (context, index) {
         ...
         return Card( // Utilise le CardTheme défini dans main.dart
            margin: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 4.0), // Marge réduite entre les cartes
           child: ListTile(
             // ====== AFFICHAGE DE L'IMAGE DE L'ACTIF ICI ======
             leading: asset.imageUrl != null && asset.imageUrl!.isNotEmpty // Vérifie si l'URL de l'image existe et n'est pas vide
                 ? CachedNetworkImage( // Utilise CachedNetworkImage pour afficher l'image depuis l'URL
                     imageUrl: asset.imageUrl!, // L'URL de l'image
                     width: 40, // Largeur de l'icône
                     height: 40, // Hauteur de l'icône
                     placeholder: (context, url) => const CircularProgressIndicator(), // Widget affiché pendant le chargement
                     errorWidget: (context, url, error) => const Icon(Icons.currency_bitcoin, size: 40, color: Colors.grey), // Icône par défaut en cas d'erreur
                   )
                 : const Icon(Icons.currency_bitcoin, size: 40, color: Colors.grey), // Icône par défaut si pas d'URL d'image
             // ===============================================
             ...
             trailing: Column( // Affiche le prix et la variation à droite
               crossAxisAlignment: CrossAxisAlignment.end,
               mainAxisAlignment: MainAxisAlignment.center,
               children: [
                 Text(_formatPrice(asset.currentPrice), // Formatage du prix
                      style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                 Text(_formatPercentage(asset.priceChangePercentage24h), // Formatage du pourcentage
                      style: TextStyle(
                        fontSize: 14,
                        color: _getPriceChangeColor(asset.priceChangePercentage24h), // Couleur dynamique (vert/rouge)
                      )),
               ],
             ),
             ...
             onTap: () {
               ...
               if (asset.id != null && asset.id!.isNotEmpty) {
                   Navigator.pushNamed(context, '/asset_detail', arguments: asset.id);
               } else {
