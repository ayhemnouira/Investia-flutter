...
class DataService {
   ...
   final String _finnhubApiKey = 'd0bvhm9r01qs9fjjltpgd0bvhm9r01qs9fjjltq0'; // Assure-toi que c'est la bonne clé si tu l'utilises pour News
   final String _finnhubBaseUrl = 'https://finnhub.io/api/v1';

   // CoinGecko Base URL (pour les données de marché crypto)
   final String _coinGeckoBaseUrl = 'https://api.coingecko.com/api/v3'; // <-- BASE URL CORRECTE POUR COINGECKO
   ...
   Future<List<NewsArticle>> fetchNewsArticlesWithCache() async {
       ...
       try {
           ...
           final List<dynamic> articlesJson = jsonResponse is List ? jsonResponse as List<dynamic> : []; // Assure-toi que c'est une liste
   ...
   Future<List<Asset>> fetchMarketAssets() async {
        ...
        try {
            ...
            final List<dynamic> assetsJson = jsonResponse is List ? jsonResponse as List<dynamic> : [];
            ...
            return assetsJson
                .where((item) => item is Map<String, dynamic>) // Filtrer les éléments non-Map
                .map((json) => Asset.fromJson(json as Map<String, dynamic>)) // Mapper vers Asset
                .toList();
        } catch(e) {
